...
p.tau_I     = 0.1667;%0.01;%0.05; %            % converted - eclipse phase
...
p.K_del_T = 0.01;
p.K_del_T_var = p.K_del_T;
p.del_I_T = 7;
p.del_I_T_var = p.del_I_T;
p.h_T = 0.5;
...
p.d_N        = 1.2773;     % Death rate of neutrophils (1/day) 
...
% T cells %-----------------------------------------------------------
p.T0         = 0;%1.104*1e-4;           % Initial number ot T cells x()??
...
p.k_lin_L = 16.636;        % renal clearance
...
p.R_G_M = 1058;             % Number of GM-CSF receptors on monocytes   % XXXXXXXXX CHANGES THIS FROM 1820 as couldn't find original reference
...
p.del_V_A = 5;
p.eps_V_A = 1000;
p.h_A = 1.19;%Hill coefficient antibody neutralization
...
%(Jenner et al. PLOS Path 2021)
p.eps_F_I = 2*1e-4;
p.p_F_I = 2.8235;
p.eta_F_I = 0.0011164*1e1;

p.p_F_M = 3.5600; %changed
