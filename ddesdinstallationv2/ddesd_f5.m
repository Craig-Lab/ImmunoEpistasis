function sol = ddesd_f5(ddefun,delays,history,tspan,options)
...
%   $Revision: 1.1.6.6 $  $Date: 2010/11/17 11:26:12 $

%-------------------------------------------------------------------------

%  This version:
%  Alexey S. Eremin, Antony R. Humphries
%  3rd November 2016

%  This version incorporates improvements in the handling of the delays to
%  reduce the computational complexity and execution time of the code to be
%  close to linear in the length of the solution time interval. 
% 
%  See <insert publication details here> for details.
...
if isnumeric(delays)
  indl = ones(1,length(delays));
else % function handle
  indl = ones(1,length(delays(0,zeros(length(y0),1))));
end

% Evaluate initial history at delayed arguments, then the slope.
[Z0,~,indl] = lagvals(t0,y0,delays,history,t0,y0,[],0,indl);
f0 = ddefun(t0,y0,Z0);
nfevals = nfevals + 1;
[m,n] = size(f0);
if n > 1
...
chunk = min(100,floor(10000/neq));
%chunk = 5600;

tchunk = zeros(1,chunk,dataType);
ychunk = zeros(neq,chunk,dataType);
%tchunk(1:chunk) = tfinal;
...
while ~done
  ...
  while true
    ...
    if Tnout > length(tout)
      chunk = max(100,ceil(1.1*(tfinal-t)/(t-tout(1))/nout));
      %tchunk = zeros(1,chunk,dataType);
      %ychunk = zeros(neq,chunk,dataType);
      %tout  = [tout, tchunk];
      %yout  = [yout, ychunk];
      ...
      %display(['length(tout) = ',num2str(length(tout))]);
    end
    tout(Tnout) = tnew;
    yout(:,Tnout) = ypred;
    ypout(:,Tnout) = yppred;
    ...
    for i = 1:2
      for j = 2:4
        ...
        [Z,implicit_step,indl] = lagvals(t+hB(j),ytemp,delays,history,...
          tout,yout,ypout,Tnout,indl);
        fnew(:,j) = ddefun(t+hB(j),ytemp,Z);
        implicit = implicit || implicit_step;
      end
      ynew = yout(:,nout) + fnew*hC;
      [Z,implicit_step,indl] = lagvals(tnew,ynew,delays,history,...
        tout,yout,ypout,Tnout,indl);
    ...
    for i = 1:2
      xm = t + h*sm(i);
      [ym,ypm] = ntrp3h(xm,tout(nout),yout(:,nout),...
        tout(Tnout),yout(:,Tnout),ypout(:,nout),ypout(:,Tnout));
      [Z,~,indl] = lagvals(xm,ym,delays,history,tout,yout,ypout,Tnout,indl);
...
function [Z,implicit_step,indl] = lagvals(tnow,ynow,delays,history,X,Y,YP,N,inds)
...
Nxint = length(xint);
if length(inds) < Nxint
  indl = zeros(1,Nxint);
  indl(1:length(inds)) = inds(:);
  indl(length(inds)+1:end) = floor(Nxint/2);
else
  indl = inds;
end
...
for j = 1:Nxint
  ...
  elseif xint(j) == X(1)   % Special case for initialization.
    ...
    %n = find(xint(j) >= X,1,'last');
    %if n == length(X)
    %    nm = n - 1;
    %    Z(:,j) = ntrp3h(xint(j),X(nm),Y(:,nm),X(n),Y(:,n),YP(:,nm),YP(:,n));
    %else
    %    n1 = n + 1;
    %    Z(:,j) = ntrp3h(xint(j),X(n),Y(:,n),X(n1),Y(:,n1),YP(:,n),YP(:,n1));
    %end;
    n = linsearch(xint(j),X,indl(j),N-1);
    n1 = n + 1;
    Z(:,j) = ntrp3h(xint(j),X(n),Y(:,n),X(n1),Y(:,n1),YP(:,n),YP(:,n1));
    indl(j) = n;
  end
...
Z = lagvals(ttry,ytry,delays,history,X,Y,YP,length(X),floor(length(X)/2));
[vtry,isterminal,direction] = eventfun(ttry,ytry,Z);
end

function n = linsearch(x0,X,i0,N)
n = i0;
if (x0 >= X(i0))
  for k = (i0+1):N
    if (X(k) > x0)
