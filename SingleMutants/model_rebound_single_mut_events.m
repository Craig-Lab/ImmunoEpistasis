function [time,sol,solstruc] = model_rebound_single_mut_events(p,tspan,lambda,effects,prod_red)
...
%------------------------------------------------------------------------
function [dydt,s_x] = ddefun(t,y,Z)
...
if TotalVL > 1e-5
    [dV1,m_V_w] = VirusRHS_single_mut(p,I1,V1,V3,V2,V4,MPhi_I,N,A,ones(1,5),effects(1,:));
    [dV2,m_V_m] = VirusRHS_single_mut(p,I2,V2,V4,V1,V3,MPhi_I,N,A,ones(1,5),effects(2,:));
    [dV3,~] = VirusRHS_single_mut(p,I3,V3,V1,V4,V2,MPhi_I,N,A,ones(1,5),effects(3,:));
    [dV4,~] = VirusRHS_single_mut(p,I4,V4,V2,V3,V1,MPhi_I,N,A,ones(1,5),effects(4,:));
else 
    V1 = 0; V2 = 0; V3 = 0; V4 = 0;
    dV1 = 0; dV2 = 0; dV3 = 0; dV4 = 0;
    m_V_w = (p.del_V_MPhi).*(MPhi_I) + (p.del_V_N).*(N) + p.d_V_spec  + ((p.del_V_A).*(A).^p.h_A)./(p.eps_V_A.^p.h_A + (A).^p.h_A);
    m_V_m = (gam_VM*p.del_V_MPhi).*(MPhi_I) + (gam_VN*p.del_V_N).*(N) + p.d_V_spec  + ((gam_VA*p.del_V_A).*(A).^p.h_A)./(p.eps_V_A.^p.h_A + (A).^p.h_A);
end

[dI1,b_I_w,m_I_w,inf_rate_w] = InfectedCellsRHS_single_mut(p,I1,ylag1(5),ylag1(1),MPhi_I,N,T,F_B,Tkillrate(1),ones(1,5),effects(1,:));
[dI2,b_I_m,m_I_m,inf_rate_m] = InfectedCellsRHS_single_mut(p,I2,ylag1(5),ylag1(2),MPhi_I,N,T,F_B,Tkillrate(2),ones(1,5),effects(2,:));
[dI3,~,~,~] = InfectedCellsRHS_single_mut(p,I3,ylag1(5),ylag1(3),MPhi_I,N,T,F_B,Tkillrate(3),ones(1,5),effects(3,:));
[dI4,~,~,~] = InfectedCellsRHS_single_mut(p,I4,ylag1(5),ylag1(4),MPhi_I,N,T,F_B,Tkillrate(4),ones(1,5),effects(4,:));

dS = p.lam_S*(1-(S+TotalInf+D+R)/p.Smax)*S-(inf_rate_w*y(1)+inf_rate_m*y(2)+p.rho*p.del_N*(N^p.h_N/(N^p.h_N+p.IC_50_N^p.h_N)))*S+p.d_R*ylag3(10);
 
%calculate spike evasion 
beta_new = p.beta;

%calculate reduced IFN effect
IFN_eff = (b_I_w*ylag1(1) + b_I_m*ylag1(2));

dR = p.lam_S*(1-(S+TotalInf+D+R)/p.Smax)*R+(IFN_eff)-p.rho*p.del_N*(N^p.h_N/(N^p.h_N+p.IC_50_N^p.h_N))*R-p.d_R*ylag3(10);
dD = (m_I_w*I1+m_I_m*I2)+(p.rho*p.del_N*(N^p.h_N/(N^p.h_N+p.IC_50_N^p.h_N)))*(S+R)-(p.d_D+(p.del_MPhi_D-p.del_D_MPhi)*(MPhi_R+MPhi_I))*D;
...
ds_x = ((sqrt(4.*p.phat.*b_I_m + (m_V_m-m_I_m).^2)-m_V_m-m_I_m)./2 - (sqrt(4.*p.phat.*b_I_w + (m_V_w-m_I_w).^2) - m_V_w-m_I_w)./2);

dydt = [dV1;dV2;dV3;dV4;dS;dI1;dI2;dI3;dI4;dR;dD;dMPhi_R;dMPhi_I;dM;dN;dT;dL_U;dL_B;dG_U;dG_B;dC_U;dC_B;dF_U;dF_B;dA;ds_x];

end

%------------------------------------------------------------------------
function s = history(t)
         s = [0;0;0;0;p.S0;0;0;0;0;p.R0;p.D0;p.MPhi_R_0;p.MPhi_I_0;p.M0;p.N0;p.T0;p.L_U_0;p.L_B_0;p.G_U_0;p.G_B_0;p.C_U_0;p.C_B_0;p.F_U_0;p.F_B_0;p.A_0;0];
end
...
function s = history_virus(t)
    if t < 0
          ...
          s = [p.V10;p.V20;p.V30;p.V40;p.S0;p.I10;p.I20;p.I30;p.I40;p.R0;p.D0;p.MPhi_R_0;p.MPhi_I_0;p.M0;p.N0;p.T0;p.L_U_0;p.L_B_0;p.G_U_0;p.G_B_0;p.C_U_0;p.C_B_0;p.F_U_0;p.F_B_0;p.A_0;0];
    end
end
